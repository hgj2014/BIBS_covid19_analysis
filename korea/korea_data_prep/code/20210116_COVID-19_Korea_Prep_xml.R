...
province <- c("Domestic", "Capital", "NonCapital")
# province <- c("전국(지역)") #, "수도권(서울+경기+인천)", "비수도권")
for(p in province){
  if(p == province[1]){
...
file_date <- paste0(c("2021",unlist(strsplit(as.character(max(as.Date(df_fin_ch$Date))), split = "-"))[2:3]), collapse = "")
write.csv(df_fin_ch, paste0(data_path, "COVID_19_Cases_Korea_Merged_Fin_", file_date, ".csv"), row.names = F, quote = F)
write.csv(df_fin, paste0(data_path, "COVID_19_Cases_Korea_Merged_", file_date, ".csv"), row.names = F, quote = F)
...
p <- ggplot(data = temp, aes(x = Date, y = Count, col = Type)) +
  ...
  labs(title = "COVID-19 Daily Cases",
       subtitle = paste0("Korea, ", as.character(min(as.Date(temp$Date))), "~", as.character(max(as.Date(temp$Date)))), x = "Date", y = "Confirmed Cases")+
  theme_bw() +
...
ggsave(p, filename = paste0(as.character(today()), "_Daily_case_in_Korea_by_Region_total.png"),path = "~/covid_19/Korea_Prep/Plot/", width = 12, height = 7, dpi = 300)
